...
export default function EditBlogPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          canonical: blog.canonical || `${DOMAIN}/blog/${blog.slug || ''}`,
          featuredImage: blog.featuredImage || '',
          featuredImageAlt: blog.featuredImageAlt || '',
          author: blog.author || blog.authorId?.username || ''
        })
      } else {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        updateFormData('featuredImage', data.url)
      } else {
  ...
  const handleSubmit = async (status: string) => {
    ...
    if (status === 'draft') setSaveLoading(true); else setSaving(true)
    try {
      const response = await fetch(`/api/blog/${params.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (status === 'draft') setSaveLoading(false); else setSaving(false)
    }
  }
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              <button onClick={() => handleSubmit('draft')} disabled={saveLoading || saving} className="px-6 py-2 text-gray-700 bg-gray-200 hover:bg-gray-300 rounded-lg transition-colors disabled:opacity-50 font-medium flex items-center gap-2">
              ...
              <button onClick={() => handleSubmit('published')} disabled={saving} className="px-6 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-lg hover:from-blue-700 hover:to-purple-700 transition-colors disabled:opacity-50 font-medium shadow-lg">
          ...
          <div className={`space-y-6 ${isFullscreen ? 'col-span-1' : 'lg:col-span-2'}`}>
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              ...
              <div className="mt-3">
                ...
                <input type="text" value={formData.slug} onChange={(e) => { const slug = e.target.value; setFormData(prev => ({ ...prev, slug, canonical: prev.canonical && prev.canonical !== `${DOMAIN}/blog/${prev.slug}` ? prev.canonical : `${DOMAIN}/blog/${slug}` })) }} placeholder="post-slug" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-800" />
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setIsFullscreen(!isFullscreen)} className="flex items-center gap-2 px-3 py-1 text-sm bg-blue-800 hover:bg-gray-700 text-white rounded-lg transition-colors">
                  {isFullscreen ? (
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-6 border border-white/20">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${formData.metaTitle.length >= 30 && formData.metaTitle.length <= 60 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${formData.metaDescription.length >= 120 && formData.metaDescription.length <= 160 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
